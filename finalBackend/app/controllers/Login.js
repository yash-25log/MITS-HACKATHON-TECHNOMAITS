...
async function Login(req, res, next) {
  ...
  try {
    if (role === "alumni") {
      ...
      const User = await BaseRepo.baseDetail(alumniTable, params);
      ...
      const college = await BaseRepo.baseDetail(collegeRegisteration, {
        searchParams: { CollegeId: User.CollegeId },
      });
    ...
    if (role === "college") {
      ...
      const token = jwt.sign(
        { CollegeId: User.CollegeId, Role: role },
        process.env.SECRET_KEY,
        { expiresIn: "365d" }
      );
